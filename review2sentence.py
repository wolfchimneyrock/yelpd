...
def connect_mongo_collection(server='localhost',port=27017,db='yelp',collection='review'):
...
def load_reviews(indb,outdb,tokenizer,skip=0,limit=0):
    ...
    for rev in indb.find({},{'_id':'0','review_id':'1','text':'1'}).limit(limit).skip(skip):
        sentences = review_to_sentences(rev['text'],tokenizer)
        for s in sentences: 
            bulk.insert({'text':s,'review_id':rev['review_id']})
    return bulk.execute()
...
batchsize=10000
totalsize=db.yelp.review.count()
...
for n in range(0,totalsize,batchsize):
    ...
    count+=1    
    result=load_reviews(db.yelp.review,db.yelp.sentence,pickle,skip=n,limit=batchsize)
    #print (result)
    print "%d sentences inserted..." % result['nInserted']
    total+=result['nInserted']
print "Read final %d reviews and tokenized ..." % finalsize,
result = load_reviews(db.yelp.review,db.yelp.sentence,pickle,skip=totalsize-finalsize)
print "%d sentences inserted..." % result['nInserted']
